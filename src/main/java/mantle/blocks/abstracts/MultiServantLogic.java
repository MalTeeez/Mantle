...
public class MultiServantLogic extends TileEntity implements IServantLogic, IDebuggable {
    ...
    public boolean hasValidMaster() {
        ...
        if (worldObj.getBlock(master.x, master.y, master.z) == masterBlock
                && worldObj.getBlockMetadata(master.x, master.y, master.z) == masterMeat)
            return true;
    ...
    public boolean verifyMaster(IMasterLogic logic, int x, int y, int z) {
        if (master.equalCoords(x, y, z) && worldObj.getBlock(x, y, z) == masterBlock
                && worldObj.getBlockMetadata(x, y, z) == masterMeat)
            return true;
        else return false;
    }
    ...
    public boolean setMaster(int x, int y, int z) {
        if (!hasMaster || worldObj.getBlock(master.x, master.y, master.z) != masterBlock
                || (worldObj.getBlockMetadata(master.x, master.y, master.z) != masterMeat)) {
            overrideMaster(x, y, z);
            return true;
        } else {
