...
public class ZipLoader {
        ...
        if (f.exists()) {
            if (FilenameUtils.getExtension(f.getPath()).equalsIgnoreCase("zip")) {
                try {
                    ...
                    while (e.hasMoreElements()) {
                        ...
                        if (!entry.isDirectory()) {
                            if (isClient && isImage(flExt)) {
                                ...
                                ResourceLocation rsLoc = Minecraft.getMinecraft().getTextureManager()
                                        .getDynamicTextureLocation(id, dynTexture);
                            ...
                            if (flExt.equalsIgnoreCase("json")) {
                                Gson g = new Gson();
                                bl = g.fromJson(
                                        IOUtils.toString(zipfile.getInputStream(entry), Charsets.UTF_8),
                                        BookLoad.class);
                                        ...
                                        if (is == null) {
                                            ...
                                            if (is != null) {
                                                if (bis.tags != null && !bis.tags.isEmpty()) {
                                                    try {
                                                        is.stackTagCompound = (NBTTagCompound) JsonToNBT
                                                                .func_150315_a(bis.tags);
                                                    } catch (Exception e1) {}
                                                }
                                ...
                                b.leftImage = bl.LeftImage == null
                                        ? new ResourceLocation("mantle", "textures/gui/bookleft.png")
                                        : MantleClientRegistry.getBookImageFromCache(bl.LeftImage).resource;
                                b.rightImage = bl.rightImage == null
                                        ? new ResourceLocation("mantle", "textures/gui/bookright.png")
                                        : MantleClientRegistry.getBookImageFromCache(bl.rightImage).resource;
                                b.itemImage = bl.BookIcon == null
                                        ? new ResourceLocation("mantle", "textures/items/mantlebook_blue.png")
                    ...
                    b.leftImage = lImg == null ? new ResourceLocation("mantle", "textures/gui/bookleft.png")
                            : MantleClientRegistry.getBookImageFromCache(lImg).resource;
                    b.rightImage = rImg == null ? new ResourceLocation("mantle", "textures/gui/bookright.png")
                            : MantleClientRegistry.getBookImageFromCache(rImg).resource;
                    b.itemImage = bIcon == null ? new ResourceLocation("mantle", "textures/items/mantlebook_blue.png")
                            : MantleClientRegistry.getBookImageFromCache(bIcon).resource;
                    b.isFromZip = true;
                    BookDataStore.addBook(b);
                } catch (Exception e) {
                ...
                logger.error(
                        "Attempted to load non-zip file for mantle book. File " + f.getName() + " will be skipped");
            }
        } else {
    ...
    public static ItemStack[] getISArray(JsonArray a) {
        ...
        for (int i = 0; i < a.size(); i++) {
            if (a.get(i).getAsString() != null && !a.get(i).getAsString().isEmpty()
                    && a.get(i).getAsString().equals("none")) {
                is[i] = null;
            } else {
